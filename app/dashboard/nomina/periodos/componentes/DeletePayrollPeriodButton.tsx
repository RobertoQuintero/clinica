...
import { deletePayrollPeriod } from "../actions";
...
export default function DeletePayrollPeriodButton({ idPeriod, periodCode }: Props) {
  ...
  async function handleConfirmDelete() {
    ...
    try {
      const result = await deletePayrollPeriod(idPeriod);
      if (!result.ok) {
      ...
      setErrorMessage("Error inesperado al eliminar el periodo");
    } finally {
      setIsDeleting(false);
    }
  }
  ...
  return (
    <>
      <button
        ...
        aria-label={`Eliminar ${periodCode}`}
      ...
      {isConfirming && (
        ...
        >
          <div
            role="alertdialog"
            aria-modal="true"
            aria-labelledby="delete-payroll-period-title"
            className="w-full max-w-sm rounded-xl bg-white dark:bg-zinc-900 p-6 text-left shadow-xl"
          >
            <h3 id="delete-payroll-period-title" className="text-base font-semibold text-[#0b1c30] dark:text-zinc-50">
              ¿Eliminar {periodCode}?
            </h3>
            <p className="mt-2 text-sm text-[#44474f] dark:text-zinc-400">
              El periodo se borra por completo y su consecutivo no se reutiliza.
            </p>
            {errorMessage && (

...
export default function CitaHoyFila({ cita, busy, onCancelar, onEmpezar }: Props) {
  return (
    <tr className="bg-white dark:bg-zinc-900 hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors">
      ...
      <td className="px-4 py-2 text-zinc-600 dark:text-zinc-300 hidden sm:table-cell">{cita.nombre_servicio_opcion ?? "—"}</td>
      <td className="px-4 py-2 text-zinc-800 dark:text-zinc-100 max-w-35 truncate">
          <span className={`rounded-full ${cita.id_tratamiento ? 'bg-amber-100 dark:bg-amber-900/40' : ''} px-2.5 py-0.5 text-xs text-amber-800 dark:text-amber-300`}>{cita.id_tratamiento?'Tratamiento':''}</span>
          </td>

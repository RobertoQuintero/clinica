...
- Module under `app/dashboard/nomina/`. It has **Periodos** (`nomina/periodos/`, spec 52), **Procesar nómina** (`nomina/procesar/`, spec 53) the per-employee **Detalle** (`nomina/procesar/[id_empleado]/`, spec 54) and **Comisiones** (`nomina/comisiones/`, spec 56); `nomina/page.tsx` redirects to Periodos. Two concepts are calculated: the base salary and the commission for attended consultas (operativa only). Other bonuses, deductions (IMSS/ISR), attendance discounts, approval/payment, receipts, dispersion and CFDI stamping are not built.
- Lives in the `payroll` schema (separate from `RH`, like `BILLING`). Besides `payroll.periods`, `payroll.period_employees` and `payroll.commission_tiers`, the schema holds catalogs not yet consumed: `cat_taxed_exempt`, `perceptions`, `tablas_retencion` (keyed by `id_payment_period` + `ejercicio`).
...
- `nomina/comisiones/page.tsx` is a Server Component listing the tiers; see the commission section above.
...
- **Perception lines:** built by `buildPerceptionLines(snapshot, fechaIngreso, fechaInicio, commissionTiers)` in `lib/payroll/perceptionLines.ts`. It returns `sueldo_base` and, when `importe_comision > 0`, `comision_consultas` (spec 56). `sueldo_base` ("N días × $X diarios", amount = `importe_salario` as stored, never recalculated in TS) with the note "Ingresó el DD/MM/AAAA, proporcional" when `fecha_ingreso > fecha_inicio` (string comparison, no `Date`). **New concepts (bonuses, …) are added there as new lines**; the card iterates the list and needs no layout change.
- **States:**
